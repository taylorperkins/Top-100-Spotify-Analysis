...
segments_timbres <- read.csv('./data/full-analysis/segments/segment-timbres.csv')
...
segments_timbres <- segments_timbres %>% 
  group_by(song_id) %>% 
  mutate(
    X0_song_avg = mean(X0),
    X1_song_avg = mean(X1),
...
# SAVE OFF R-OBJECT
#############################
saveRDS(segments_timbres, 'data/r-objects/segments-timbres/segments-timbres.rds')
